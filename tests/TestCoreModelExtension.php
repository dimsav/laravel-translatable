...
use Approached\Translatable\Test\Model\Company;
use Approached\Translatable\Test\Model\Continent;
use Approached\Translatable\Test\Model\Country;
use Approached\Translatable\Test\Model\CountryGuarded;
use Approached\Translatable\Test\Model\CountryStrict;
use Approached\Translatable\Test\Model\CountryTranslation;
use Approached\Translatable\Test\Model\Vegetable;
...
class TestCoreModelExtension extends TestsBase
{
    ...
    public function test_it_returns_false_if_exists_and_dirty_and_parent_save_returns_false()
    {
        ...
        $event->listen('eloquent*', function ($model) use ($that) {
            return get_class($model) == 'Approached\Translatable\Test\Model\Country' ? false : true;
        });
    ...
    public function test_it_returns_false_if_does_not_exist_and_parent_save_returns_false()
    {
        ...
        $event = App::make('events');
        $event->listen('eloquent*', function ($model) use ($that) {
            return get_class($model) == 'Approached\Translatable\Test\Model\Continent' ? false : true;
        });
